...
### Phase 5: Review the Plan Against Documentation

After generating the plan, **validate it against the Hindsight documentation** before presenting it to the user. Invoke the `/hindsight-docs` skill and use the reference docs to verify:

1. **API parameters** — Check that retain, recall, reflect, and mental model calls use correct parameter names, types, and valid values. Cross-reference against the API docs (retain.md, recall.md, reflect.md, mental-models.md, memory-banks.md).
2. **SDK method signatures** — Verify the code examples match the actual SDK for the user's language. Check the Python SDK docs (python.md) or Node.js SDK docs (nodejs.md).
3. **Configuration values** — Confirm that disposition ranges, tags_match modes, budget values, and trigger options are valid.
4. **Patterns** — Verify that the retain/recall/mental model patterns follow documented best practices (e.g., document_id upsert behavior, tags_match semantics, mental model auto-refresh triggers).

If you find discrepancies, fix the plan silently — don't present errors to the user. The plan they see should be accurate.

After the review, tell the user:

"Your plan is ready — validated against the current Hindsight API documentation. Before anything works:
...
After presenting the plan (and enterprise outreach, if applicable), ask via `AskUserQuestion`:

> Ready to implement this plan?

Options:
- **Yes, implement it** — Execute the implementation checklist step by step in this session
- **Not now** — Keep the plan for later reference

If the user selects **Yes, implement it**, work through the Implementation Checklist from the plan. Execute each code block in order: install the SDK, add environment variables, initialize the client, create the bank, add retain/recall calls at the identified code locations, create mental models if applicable, and run the test scenario. Commit logical units of work as you go.
