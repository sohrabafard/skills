...
def main() -> int:
    ...
    try:
        ...
        count = 0
        for kind in ("pin", "wrapper", "manifest", "missing-owner", "sibling-pin-bypass"):
            ...
            expected = 2 if kind == "missing-owner" else (0 if kind == "sibling-pin-bypass" else 1)
            if result.returncode != expected:
                print(f"{kind}: source validator expected {expected}, got {result.returncode}", file=sys.stderr)
                return 1
            for installer in (("agents",) if kind == "sibling-pin-bypass" else ("agents", "skill")):
                target = source.parent / f"{args.shell}-{installer}-target"
                agent_target = source.parent / f"{args.shell}-{installer}-agent-target"
                agent_source = source / ("candidate-agents" if kind == "sibling-pin-bypass" else "agents")
                if args.shell == "powershell":
                    ...
                    if installer == "agents":
                        command += ["-SourceDirectory", str(agent_source), "-TargetDirectory", str(target)]
                    else:
                    ...
                    command += ([agent_source.as_posix(), target.as_posix(), owner.as_posix()]
                ...
                if kind == "sibling-pin-bypass":
                    proof = "SourceDirectory must be the validated canonical agents directory"
