...
4. If integration is explicitly authorized, bring the base branch into the work branch before verifying. Otherwise verify the current candidate and report integration as pending. Then dispatch the exhaustive tier once, on that tree: the whole suite under its normal configuration, then the race, end-to-end, and highest-level proofs the claims require. This result is always fresh and is never cited from an earlier run. Run it after documentation lands, so nothing writes to the tree afterwards, and record the base commit it observed — Phase F compares against that commit to decide whether the evidence still describes the tree being merged.
