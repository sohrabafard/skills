...
canonical_source="$(cd -- "$source_root/agents" && pwd -P)"
if [[ "$source_dir" != "$canonical_source" ]]; then
  echo "SourceDirectory must be the validated canonical agents directory; target was not changed" >&2
  exit 1
fi
policy_root="${3:-$(dirname "$source_root")/alaa-prompting-guide}"
...
"${python_cmd[@]}" -B "$source_root/scripts/validate_pack.py" --policy-root "$policy_root"
...
"${python_cmd[@]}" -B "$script_dir/check_agent_grants.py" --materialize "$source_dir" "$materialized_dir" || {
  code=$?
