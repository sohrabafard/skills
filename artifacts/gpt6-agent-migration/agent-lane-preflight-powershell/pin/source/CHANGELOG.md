...
- Require explicit installation and commit authority; activation only inspects availability.
...
- Use canonical owner policy for agent pins; add a deep correctness-review profile rendered with the standard profile from one contract. Validate deterministic wrappers and version/hash manifest before installation.
