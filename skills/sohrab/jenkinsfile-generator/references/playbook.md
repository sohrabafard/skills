# jenkinsfile-generator — full playbook
...
**NOTE:** When `parallelsAlwaysFailFast()` is set in options, explicit `failFast true` on individual parallel blocks is
redundant.
...
- **Use manual generation for:** Complex pipelines with multiple features (parallel tests + security scanning + Docker +
  K8s deployments), custom logic, or non-standard requirements
...
**Plugins covered in common_plugins.md:** Git, Docker, Kubernetes, Credentials, JUnit, Slack, SonarQube, OWASP
Dependency-Check, Email, AWS, Azure, HTTP Request, Microsoft Teams, Nexus, Artifactory, GitHub
