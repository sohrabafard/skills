...
In app-vite v2 the wrapper is `#q-app/wrappers` (not the legacy `quasar/wrappers`). During the v3 migration the import becomes `#q-app` — do not switch it on a v2 repo. The boot params (`app`, `router`, `store`, `ssrContext`, `redirect`, and in v3 also `urlPath`/`publicPath`) are documented in `$quasar-skill-packe` reference 11.
