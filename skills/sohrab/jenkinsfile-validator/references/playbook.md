# jenkinsfile-validator — full playbook
...
Comprehensive toolkit for validating, linting, and testing Jenkinsfile pipelines (both Declarative and Scripted). This
...
**Key Point**: Always use `validate_jenkinsfile.sh` as the main entry point - it orchestrates all other scripts
automatically.
...
**Important**: Plugin documentation lookup is Claude's responsibility (not automated in scripts). After running
validation, Claude should identify unknown plugins and look them up.
...
5. **Provide inline fix suggestions** when errors are found (do not use AskUserQuestion - include corrected code
   snippets directly in the response)
