...
4. **Runner and provider behaviour** — service containers, caching, artifacts, masked variables, retry syntax, CI lint — belongs to `/alaa-gitlab-ci-cd` (`$alaa-gitlab-ci-cd`) `references/00-source-map.md`. This skill holds no provider documentation link and no provider YAML, so that one place stays correct.
