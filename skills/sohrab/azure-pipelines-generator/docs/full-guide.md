...
3. Reference `./yaml-schema.md` for YAML structure
4. Reference `./best-practices.md` for implementation patterns
5. Reference `./tasks-reference.md` for common tasks
...
- User requests: "Create a full CI/CD pipeline...", "Build multi-stage pipeline...", "Deploy to multiple
  environments..."
...
4. Use `./yaml-schema.md` for stage/job/step hierarchy
...
4. Reference `./tasks-reference.md` for Docker@2 task
...
4. Reference `./tasks-reference.md` for Kubernetes tasks
...
2. Reference `./tasks-reference.md` for language-specific tasks
...
3. Reference `./templates-guide.md` for template syntax
...
1. **ALWAYS check local documentation first (REQUIRED):**
   Local docs are sufficient for most common tasks and should be your primary reference:
    - `./tasks-reference.md` - Contains .NET, Node.js, Python, Go, Docker, Kubernetes, Azure tasks
    - `./yaml-schema.md` - Complete YAML syntax reference
    - `./best-practices.md` - Security, performance, naming patterns
...
Reference `./best-practices.md` for comprehensive guidelines. Key principles:
...
1. **Security First:**
    ...
    - **Docker image tagging strategy:**
        - When **pushing** images: Use build-specific tag as primary (e.g., `$(Build.BuildId)`), optionally add
...
2. **Version Pinning:**
    ...
    - **Note on @0 versions:** Some tasks only have @0 as their current/latest major version (e.g., `GoTool@0`,
      `NodeTool@0`, `KubernetesManifest@0`). Using @0 for these tasks is correct and acceptable - the goal is to use the
      latest available major version, not to avoid @0 specifically.
...
**IMPORTANT:** When generating pipelines, **explicitly read** the relevant example files to ensure consistent patterns
and best practices. Use the Read tool to load these files before generating.
...
2. ✅ Reference resources:
    - Check `./yaml-schema.md` for multi-stage structure
    - Check `./tasks-reference.md` for NodeTool, Docker, Kubernetes tasks
    - Check `./best-practices.md` for pipeline patterns
