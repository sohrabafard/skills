...
A **counter** is a cumulative metric that only increases over time (or resets to zero on restart). Counters are used for
counting events.
...
A **gauge** is a metric that represents a single numerical value that can go up or down. Gauges represent current state
or level.
...
A **summary** is similar to a histogram but calculates quantiles on the client side and streams pre-calculated
percentiles to Prometheus.
