...
Vector matching enables combining data from different metrics. Essential for enriching metrics with metadata and
correlating related time series.
...
Use `group_left` when the left side has more time series than the right side. The result includes labels from both
sides.
