...
This skill provides comprehensive validation for bash and shell scripts, checking for syntax errors, best practices,
security vulnerabilities, and performance optimizations. It automatically detects whether a script is bash or POSIX sh
based on the shebang and applies appropriate validation rules.
...
3. **Reference documentation** for detailed explanations:
    - For ShellCheck codes: Read `./shellcheck-reference.md`
    - For common mistakes: Read `./common-mistakes.md`
    - For bash-specific issues: Read `./bash-reference.md`
    - For POSIX sh issues: Read `./shell-reference.md`
...
3. Read ./common-mistakes.md for fix patterns
4. Read ./shellcheck-reference.md for SC error explanations (if needed)
...
The validator works without ShellCheck but provides enhanced validation when it's available. The wrapper provides
automatic installation with caching for faster subsequent runs.
...
**Note**: This skill automatically loads relevant documentation based on the validation results, providing Claude with
the necessary context to explain issues and suggest fixes effectively.
