...
13. For backend authorization, user-role handling, `X-User-Roles`, role-derived policies, or access-level decisions, read `references/28-backend-permission-authorization-and-role-freeze.md`. While its provisional freeze is active, backend services must authorize with catalog-owned permission bits from trusted `X-Access` and must not introduce new role-based decision logic.
14. For auth TOTP setup, QR/`otpauth_uri`, optional enrollment, forced route-level MFA, `AUTH_TOTP_ENABLED`, `require_totp:<purpose>`, signed step-up proof tokens, local proof caching, or gateway `X-TOTP-Proof` verification, read `references/32-auth-totp-and-step-up-contract.md`; pair it with `$alaa-trust-gateway-auth` and `$alaa-frontend-developer` when public clients, SDKs, or gateway route behavior are involved.
...
- While the provisional backend role freeze is active, do not use user roles for authorization or any other runtime decision. Enforce access with catalog-owned permission bits from trusted `X-Access`, plus the contract-defined OpenFGA decision where applicable.
- Do not add new backend role resolvers, role-derived policies, role-to-permission mappings, or role-dependent behavior until this skill explicitly replaces the freeze with a finalized role contract. A backend may retain normalized roles only as non-authoritative observability or future-use metadata.
...
- provisional backend user-role freeze, permission-bit authorization, passive role metadata, and the explicit activation gate for future role semantics:
  - `references/28-backend-permission-authorization-and-role-freeze.md`
