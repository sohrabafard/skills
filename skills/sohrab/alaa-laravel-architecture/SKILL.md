...
description: "Laravel layer map and legal call graph for Ala services (Controller to Service to Repository to DB): what may cross a boundary, where the cache seam, error envelope, degraded response and domain-event emission are produced, DTO-versus-Resource-versus-Model exposure, provider register-versus-boot placement, and an executable gate for layer violations, public-id leaks and provider I/O. Use when a change crosses two layers or alters the public surface: adding or moving a route, controller, service, repository, DTO, resource, policy, provider binding or emitted event. Do not use for a rule verifiable by reading one class alone (naming, types, method length, patterns inside a file) - that is /alaa-php-clean-code ($alaa-php-clean-code). Route design-before-code to /alaa-system-design; envelope, code and metric names to /alaa-services-contract; timeout, retry and page-size values to its failure-load contract file; degradation doctrine to /alaa-reliability-sla; cache policy to /alaa-data-layer."
---
