...
- `QUploader`: queue, transport customization, drag/drop, lifecycle. It needs a backend; validate native `accept`; key hooks are `headers`, `form-fields`, `with-credentials`, `factory`; `batch` changes multi-file transport; custom header/list slots often need `QUploaderAddTrigger`.
...
Virtualize when an already-loaded list is expensive to render; use infinite scroll for incremental loading. For `QVirtualScroll`, tune `virtual-scroll-item-size`/`virtual-scroll-slice-size`; ensure custom `scroll-target` exists and scrolls; keep `items-fn` synchronous; freeze or avoid reactivity on large arrays when possible.
