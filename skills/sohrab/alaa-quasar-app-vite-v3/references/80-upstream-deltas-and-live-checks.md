...
**Stable-first rule:** for production, recommend stable releases. Since `3.0.1` (2026-07-07) that means **v3 is the default for new apps**. v2 repos migrate deliberately via `10-v2-to-v3-migration.md`, not as a side effect of another task; until migrated, keep them pinned to `^2`.
...
## `@quasar/app-vite` v3 breaking changes (summary — full playbook in `10-v2-to-v3-migration.md`)

v3 is **stable** since `3.0.1` (2026-07-07). This is a **summary** so you can pick the right code shape from within this pack. The **v2->v3 migration playbook lives in `10-v2-to-v3-migration.md`** (with the deep verified delta list in `11-review-and-upgrade-checklist.md`) — use those for any migration/upgrade work; migrations run deliberately, never as a side effect.
...
For the exact per-line `quasar.config`/boot/component shapes, use the cookbook in `22-cli-cookbook-and-examples.md`.
