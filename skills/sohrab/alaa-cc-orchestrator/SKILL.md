...
1. Resolve SKILL_ROOT as the directory containing this SKILL.md. Compare the content of `~/.claude/agents/.alaa-cc-orchestrator.version` with `SKILL_ROOT/VERSION`, and confirm every `SKILL_ROOT/agents/*.md` has a same-named file in `~/.claude/agents/`. When both hold, the agents are current — skip the rest of this section silently, with no content comparison and no setup narration. The presence check is what keeps a deleted agent file from being reported as installed for as long as the sentinel matches.
