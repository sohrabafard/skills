...
description: "Keyset (cursor) pagination design for Ala list routes: deciding whether a route paginates and by which sorts, deriving an ordering tuple whose final component is unique, building the composite index that serves it, defining a signed cursor that carries its own filter and sort context, and handling nullable columns, mutable sort values, and backward traversal. Use when adding or reviewing a list endpoint, feed, comment thread, notification list, activity log, or infinite-scroll surface; when a list repeats or drops rows between pages; when a list query is slow on later pages or its index is unused; when choosing between cursor, offset, and chunked traversal; when a cursor must be rejected; and before claiming backward pagination works. Do not use for: the collection envelope, the error envelope, and identifier exposure, which are /alaa-services-contract ($alaa-services-contract); index and query mechanics in general, which are /alaa-data-layer ($alaa-data-layer); why an unbounded result set is a defect, which is /alaa-algorithms-data-structures ($alaa-algorithms-data-structures); cursor tampering as a trust-boundary concern, which is /alaa-security-review ($alaa-security-review); what makes a test a test, which is /alaa-testing-strategy ($alaa-testing-strategy); model and effort selection, which are /alaa-prompting-guide ($alaa-prompting-guide)."
---
