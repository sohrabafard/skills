...
description: "Mandatory Vue 3, Quasar, Vite and TypeScript clean-code contract for writing, reviewing and refactoring frontend code: Vue style-guide gates, script-setup with type-only props and emits, composable and Pinia shape, SOLID and code-smell repair, design-pattern selection, TypeScript depth (discriminated unions, generics, satisfies, branded types, strict flags), hard size budgets, and the Vue-shaped bindings for async failure, security, observability and testing. Use before changing any .vue or .ts file, composable, store, boot file or route, and when code is duplicated, weakly typed, monolithic or over budget. Do not use for backend, infrastructure or CI work with no frontend impact; for Quasar API lookup (/alaa-quasar-app-vite-v3); for SSR auth, PWA policy or Web Vitals (/alaa-frontend-developer); or for design tokens, theming and motion (/alaa-ui-ux-design-system)."
---
