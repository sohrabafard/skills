...
description: "Security review gate for production multi-tenant services: trust boundaries, authentication and authorization, tenant isolation, injection, XSS, SSRF, file upload and serving, secrets and cryptography, race conditions, and fail-closed behaviour. Emits a per-item PASS/FAIL/N-A verdict with a prioritized remediation plan. Use when reviewing or writing an authn/authz change, a new endpoint, webhook, or queue consumer, a sanitisation path, a query that can cross tenants, a request-influenced outbound URL, an upload or download path, or a token, session, or step-up change, and before merging any change that moves a trust boundary. Do not use for style cleanup, docs-only work, or performance tuning with no trust impact. Route availability-side degradation and retries to /alaa-reliability-sla; token and trusted-header issuance to /alaa-trust-gateway-auth; security-event and telemetry design to /alaa-observability-soc."
---
