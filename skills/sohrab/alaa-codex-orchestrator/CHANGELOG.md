...
- Added execution profiles. `lean`, `standard`, and `hardened` size the pipeline to the finished plan, so a bounded one-lane change no longer pays the full six-phase cost. `lean` drops the reviewer dispatch, never the review: the main thread did not write the diff, so it remains an independent authority over it. Escalation is one-way, and `references/verification-and-gates.md` carries the profile-to-gate mapping.
