...
description: "Reliability doctrine for services under a 99.99%+ SLA: request deadlines and timeouts, retries with jittered backoff and retry budgets, circuit breakers and bulkheads, admission control and load shedding, graceful degradation, the full idempotency contract including two concurrent requests carrying the same key, error budgets and SLOs, and the fault injection that proves each mechanism fired. Use when adding or reviewing an outbound call, a retry, a timeout, a connection or worker pool bound, a cache fallback, an admission or shedding decision, a repeatable write, or an SLO; when a dependency is slow, flapping, or gone; and when deciding what a request does with a failing dependency. Do not use for: exact Ala timeout, retry, pool, or shed values, which are /alaa-services-contract references/22; whether a caller may act, or any control that must fail closed, which is /alaa-security-review; telemetry-plane reliability and burn-rate alert authoring, which is /alaa-observability-soc; broker prefetch, acknowledgement, and DLQ mechanics, which is /alaa-async-messaging; model and effort selection, which is /alaa-prompting-guide."
---
