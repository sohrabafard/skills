...
Read when a route saturates, a queue grows, an in-flight limit is chosen, or a rate limit and a concurrency limit are being confused for each other. `SKILL.md` requires bounded queues, synchronous shedding, durable acceptance for work that must survive, and protected probes. This file owns admission and ordering; `10-deadlines-and-timeouts.md` owns the accepted-work lifecycle. `/alaa-services-contract` `references/22-failure-load-and-deprecation-contract.md` supplies in-flight defaults, status codes, and event names.

## The decision: shed or queue

**First check persisted ownership.** Once work is durably accepted, use the lifecycle in `10-deadlines-and-timeouts.md` regardless of whether the caller received a receipt or is still waiting. Under saturation, bound processing and apply backpressure; do not shed that responsibility.

Before durable acceptance, two questions decide admission, in this order.

1. **Is someone waiting for this right now?** A caller holding an open connection with a deadline is waiting. Receipt delivery alone says nothing about persisted ownership.
...
- Waiting **and** unrecoverable → **change the interface, not the limit.** Persist ownership, return an application acceptance receipt, and process asynchronously under the owner's lifecycle contract. The failure mode of getting this wrong is a request waiting for capacity while holding a resource — the queue-at-ingress failure below.
- Neither waiting nor unrecoverable → drop it and count the drop. Prefetches, speculative work, and telemetry batches live here.

**Reject overload before durable acceptance; never report acceptance when capacity or storage prevented persistence.** An uncertain commit follows identity-based reconciliation in `10-deadlines-and-timeouts.md`, not a claim that acceptance failed. Admission failure cannot be hidden behind a receipt.

For request-bound work, **a queue does not add capacity, it converts rejection into latency.** The caller's deadline does not move because work was queued, so a queued request either completes in time — meaning there was capacity and the queue was unnecessary — or completes after the caller has left, meaning the capacity was spent producing a result nobody reads. Shedding gives the caller the one piece of information it can act on: not now.

## Why an unbounded queue is a latency bomb

The arithmetic is not subtle. Wait time is depth divided by service rate. While arrivals exceed the service rate, depth grows monotonically, so **every item's wait grows without bound**. A request-bound queue can then report success after callers' deadlines; durable acceptance metrics can look healthy while completion stalls. Neither measures useful completion.

For request-bound work, capacity is spent twice: the queued item is processed, its result discarded because the caller is gone, and the caller's retry is processed too. An overloaded system with an unbounded queue therefore gets *further* behind under retry pressure, which is why this failure does not self-correct.

Bounding depth is only half. **At dequeue, check the applicable lifetime, not only at enqueue.** Drop stale caller-bound work before processing; for accepted work, check business validity and cancellation and follow the owner-defined disposition in `10-deadlines-and-timeouts.md`. The HTTP caller's departure is not that disposition.
...
For work not durably accepted, shed in decreasing order of cheapness and increasing order of value to the caller's outcome. The order below is the default, and a route may only move within it for a recorded reason.

1. **Caller-bound work whose request deadline has expired.** Free to shed at a queue or admission point, since it cannot produce a usable response.
...
Tell the client the truth: a distinct rejection status with a retry hint, and a distinct event, so a shed is never confused with a dependency failure or an application error in a dashboard. `/alaa-services-contract` owns the code and event names; `/alaa-observability-soc` owns their requirement level.
...
- **A producer that cannot be slowed** — a third-party webhook, a fixed-rate sensor feed — is admitted to a durable bounded buffer or rejected at its edge before ownership is accepted, with a recorded event rather than a silent overwrite. There is no configuration that makes an unslowable producer safe without a bound.

An in-process channel or buffer between stages takes the same capacity and waiting bounds, with disposition determined by the ownership boundary above. Unbounded internal channels are where a bounded ingress limit leaks: the request is admitted, hands work to an unbounded internal stage, and the memory the admission limit was protecting is consumed anyway.
