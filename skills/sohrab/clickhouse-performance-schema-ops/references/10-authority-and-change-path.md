...
contains a `clickhouse/ddl/` directory creating the analytics database and its tables, a `vector/`
topology whose sinks write into those tables, and a `docs/DECISIONS.md` in which each schema choice
is ratified with its reason. On this fleet that repository creates the `wa_raw` database with
`wa_raw.events_raw` and `wa_raw.watch_segments_raw` (`clickhouse/ddl/001_init.sql:15-151`). Every
DDL right lives there: database and table creation, column type, `ORDER BY`, `PARTITION BY`,
engine, codec, retention.
...
| --- | --- | --- |
| the ingest-pipeline repository | DDL, plus the `docs/DECISIONS.md` entry that ratifies it, plus the reviewer output from `scripts/review_clickhouse_ddl.py` | a change to a consumer's Go code |
...
`clickhouse/ddl/001_init.sql:13` records the current rule for this pipeline: "This is
...
pipeline's own reset flag (`docs/DECISIONS.md:147-154`, "Local WA Database Re-Initialization")
exists because of exactly this, and it is scoped to local use.
...
## Retention is an open decision on this pipeline, not a default

`clickhouse/ddl/001_init.sql:9` and `docs/DECISIONS.md:13-18` state "NO TTL by design (retention
policy is a later decision)", with the reason that retention is a policy choice that must not be
hardcoded in bootstrap DDL and that a TTL risks deleting data during early iteration. Treat this as
an unresolved decision owned by the pipeline owner, not as an omission to fix. When a task needs a
retention answer, produce the retention options and their storage consequences and hand them to the
owner; do not add a TTL clause to close the gap yourself. Mechanism choice, once the policy exists:
`50-mvs-projections-and-ttl.md`.

## Single node today, cluster later

Both tables are `ENGINE = MergeTree`, with `ReplicatedMergeTree` templates present but commented
out (`clickhouse/ddl/001_init.sql:84-88` and `:144-148`). Verified against the official replication
