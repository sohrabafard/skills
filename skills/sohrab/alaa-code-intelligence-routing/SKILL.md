...
- Trust a code graph's edges according to the provenance it returns, and read its silence as a limit rather than a fact. A graph may resolve more than direct static calls — callback registration and interface-to-implementation are commonly real edges — so an empty text search never establishes that a symbol is unused. It may not reliably resolve a dispatch computed at runtime; where it detects such a site, it reports the boundary instead of guessing an edge. Detection is best-effort, so an unexplained gap in a path is itself a boundary.
...
- **Language-native semantic owner:** where a stack skill names its own semantic interface, that skill owns the choice and this one follows it. For Go, enable Serena and use it as the agent-facing owner for known-symbol semantics and symbol-scoped edits. Serena's Go backend uses `gopls`; direct `/golang-gopls` in Claude Code or `$golang-gopls` in Codex is allowed only for one recorded unavailable, unhealthy, or missing build-aware operation. Structural discovery stays with CodeGraph, and Go build, vet, and test commands remain the proof.
