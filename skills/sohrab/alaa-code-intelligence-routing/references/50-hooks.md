...
- Auto-approval does not broaden the selected permission mode, but it suppresses per-call prompts for matched Serena operations inside that mode. It emits an allow decision only while the client already reports a permissive permission mode and stays silent otherwise. Keep the vendor's own matcher; the authority boundary for a read-only lane belongs in that lane's tool grant, which is where `references/80-agent-scoping.md` puts it, not in a hook matcher.
