...
description: "Ownership and debug governance for `service-runtime-kit`-generated local Docker Compose and Swarm runtime in Laravel or PHP service repositories. Use when working in a repo that consumes `service-runtime-kit`, or for changes to `runtime/*.env`, `runtime/hooks/**`, `runtime/env.*.extra`, `scripts/runtime/**`, generated `docker-compose*.yml`, generated `scripts/docker/**`, generated `docker/octane/**`, generated `docker/pgbouncer/**`, `.gitattributes`, `.githooks/**`, copied helpers such as `scripts/validate_runtime.php`, runtime-kit version pinning, bootstrap or auto-fetch behavior, Windows Git Bash path-conversion of slash-valued Compose env vars such as `RABBITMQ_VHOST=/`, or which layer owns a runtime fix or debug path. Do not use for Kubernetes, OpenShift, or Helm deployment (`/caas-arvan-kuber`), GitLab CI/CD pipelines owned by the `service-ci-kit` repository (`/alaa-gitlab-ci-cd`), the normative Ala shared-infra and deployment contract (`/alaa-services-contract`), or application logic unrelated to runtime generation."
---
