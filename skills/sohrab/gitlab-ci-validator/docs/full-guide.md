...
Comprehensive toolkit for validating, linting, testing, and securing GitLab CI/CD pipeline configurations (
.gitlab-ci.yml files). Use this skill when working with GitLab CI/CD pipelines, validating pipeline syntax, debugging
configuration issues, implementing best practices, or performing security audits.
...
- Review `./gitlab-ci-reference.md` for syntax reference
- Check `./common-issues.md` for known issues
- Consult `./best-practices.md` for recommended patterns
...
**Remember**: This skill validates GitLab CI/CD configurations but does not execute pipelines. Use GitLab's CI Lint tool
or `gitlab-ci-local` for testing actual pipeline execution.
