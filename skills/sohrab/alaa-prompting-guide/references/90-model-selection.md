...
- `$alaa-codex-orchestrator` (Codex) and `/alaa-claude-orchestrator` (Claude Code): goal-scoped advisor/orchestrator mode with pinned role subagents — Sol `high` lead and reviewer, Terra `high` implementer (Sol `high` escalation for architecture-heavy lanes), and Luna `high` documenter in Codex; Fable 5 `high` (or Opus 4.8) lead with Sonnet 5 `xhigh` implementer (Opus 4.8 `xhigh` escalation for architecture-heavy lanes), Opus 4.8 `xhigh` reviewer, Sonnet 5 `high` documenter in Claude Code — plus a review gate and parent reconciliation. Prefer these for per-goal multi-model delegation; the lead is always the session's own model.
