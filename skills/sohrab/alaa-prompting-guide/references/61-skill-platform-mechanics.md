...
- **Plugin validation rejects a `description` longer than 1,024 characters.** It fails the build, so it is the number to write against. Measure the *packaged* description rather than the line on disk: plugin packaging rewrites every cross-skill call in a packaged Markdown file, frontmatter included, into its namespaced form, so each routing reference costs the namespace plus a colon and the packaged length exceeds the on-disk length. The margin to leave below 1,024 is therefore a property of the packaging namespace and not a constant — renaming the plugin moves it. Derive an author target rather than recalling one: subtract, from 1,024, the namespace length plus one for the colon, multiplied by the number of routing references the description carries, and leave room for one more reference than it has today. At this pack's current two-character namespace that arithmetic gives a target of 975, which is also what its validator enforces; a longer namespace lowers it, and a description with several routing references sits nearer the limit than a longer description with none.
