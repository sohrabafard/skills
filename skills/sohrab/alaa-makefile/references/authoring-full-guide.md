...
**Do NOT use for:** validation-only review without changes, low-signal `make -d` debugging with no design question, or
running builds.
...
1. **ALWAYS consult internal docs/ FIRST using the Read tool** (primary source of truth):
   ...
   | Requirement | Read This Doc |
      |-------------|---------------|
   | Docker/container targets | `./patterns-guide.md` (Pattern 8: Docker Integration) |
   | Multi-binary projects | `./patterns-guide.md` (Pattern 7: Multi-Binary Project) |
   | Go projects with version embedding | `./patterns-guide.md` (Pattern 5: Go Project) |
   | Parallel builds, caching, ccache | `./optimization-guide.md` |
   | Credentials, secrets, API keys | `./security-guide.md` |
   | Complex dependencies, pattern rules | `./patterns-guide.md` |
   | Order-only prerequisites | `./optimization-guide.md` or `./targets-guide.md` |
   | Variables, assignment operators | `./variables-guide.md` |

   **CRITICAL:** You MUST explicitly use the Read tool to consult relevant docs during generation, even if you have
   prior knowledge. Do NOT rely on context from earlier in the conversation. This ensures patterns are always current
   and correctly applied.

   **Required Workflow Example (Docker + Go with version embedding):**
   ```
   # Step 1: Use Read tool to get Go pattern
   Read: ./patterns-guide.md (find Pattern 5: Go Project)

   # Step 2: Use Read tool to get Docker pattern
   Read: ./patterns-guide.md (find Pattern 8: Docker Integration)

   # Step 3: Use Read tool for security considerations
   Read: ./security-guide.md (credential handling for docker-push)

   # Step 4: Generate Makefile combining patterns
   # Step 5: Document which docs were consulted in Makefile header
   ```

   **Important:** Internal docs contain vetted, production-ready patterns. Always read the relevant docs before external
   lookups.
...
3. **Fallback to WebSearch** (only if pattern not found in internal docs OR context7):
   ...
   ```
   **Trigger WebSearch when:** Internal docs don't cover the specific integration AND context7 returns no relevant
   results.
...
**CRITICAL: Always validate using the bundled `alaa-makefile` validation workflow.**

```
1. Generate Makefile following stages above
2. Run `scripts/validate_makefile.sh` from this skill
...
The mbake validator may report warnings for valid GNU Make special targets. These are false positives and can be
ignored:
...
**Note:** When validation shows info items about error handling or parallel safety, you MUST address them for any
Makefile containing Docker, deploy, or install targets. Explain in your response which patterns were applied.
...
**Note:** Manual generation following the Stage 3 patterns produces equivalent results but allows for more
customization.
